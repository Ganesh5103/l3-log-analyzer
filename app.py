...
# ---------------------------------------------------------------
# Force UTF-8 on stdout/stderr so emoji in print() calls never
# raise UnicodeEncodeError on Windows (cp1252 consoles).
# Replacing the TextIOWrapper directly via .buffer is more robust
# than reconfigure() which can silently fail in PyInstaller exes.
# ---------------------------------------------------------------
import io as _io
...
# =================================================================
# PYINSTALLER-COMPATIBLE PATH HELPERS
# =================================================================

def _resource_path(relative: str) -> str:
    """
    Absolute path to a read-only bundled resource (templates, static …).
    Uses APP_RESOURCE_DIR env var (set by main.py before import), then
    falls back to sys._MEIPASS (PyInstaller) or the directory of this file.
    """
    base = (
        os.environ.get("APP_RESOURCE_DIR")
        or getattr(sys, "_MEIPASS", os.path.dirname(os.path.abspath(__file__)))
    )
...
def _data_dir() -> str:
    """
    Writable directory beside the .exe (or script) for user data:
    uploads/, generated_logs/, converted/.
    Uses APP_DATA_DIR env var (set by main.py before import).
    """
...
_BASE_DATA = _data_dir()
UPLOAD_FOLDER = os.path.join(_BASE_DATA, "uploads")
GENERATED_LOGS_FOLDER = os.path.join(_BASE_DATA, "generated_logs")
CONVERTED_FOLDER = os.path.join(_BASE_DATA, "converted")
...
app.config['SECRET_KEY'] = os.environ.get('FLASK_SECRET_KEY', 'l3-log-analyzer-local-key-2026')
...
# Current session folder path (for RRM correlation persistence)
...
def merge_logs_for_ue_journey(folder):
    ...
    # Get list of files to process (ONLY L3 files, RRM handled separately by correlator)
    files_to_process = []
    total_size_mb = 0
    for filename in sorted(os.listdir(folder)):
        fn_lower = filename.lower()
        # ONLY process L3 files here - RRM files are handled by l3_rrm_correlator
    ...
    print(f"\n{'='*80}")
    print(f"🔍 merge_logs_for_ue_journey scanning: {folder}")
    ...
    # CRITICAL: If filter metadata is already set (from SCP/upload), preserve filtered_total
    # and do NOT overwrite it with folder scan results. The folder should only contain
    # the filtered files anyway, but this prevents edge cases where unexpected files appear.
    if analysis_progress.get('filter_active', False) and analysis_progress.get('filtered_total', 0) > 0:
        # Filter was applied during download/upload - use that count, not folder scan
        print(f"ℹ️  Using filtered count from download: {analysis_progress['filtered_total']} (folder has {len(files_to_process)})")
        analysis_progress['total'] = analysis_progress['filtered_total']
    else:
        # No filter was applied, or this is a fresh analysis - use folder scan count
        print(f"ℹ️  No filter metadata found, using folder scan count: {len(files_to_process)}")
...
# =================================================================
# TIME-RANGE FILTERING HELPERS (O(n), no assumptions on filenames)
# =================================================================
...
def _parse_timestamp_from_filename(filename):
    """
    Extract _YYYYMMDD_HHMMSS timestamp from a filename and return a datetime.
    Returns None when the pattern is absent (file has no embedded timestamp).
    Raises ValueError when the pattern is present but the values are invalid.
    """
    ...
    try:
        ...
        raise ValueError(
            f"Invalid timestamp values in '{os.path.basename(filename)}': {exc}"
        )


def _filter_files_by_time_range(file_list, start_dt, end_dt):
    """
    Filter *file_list* (paths or plain names) to those whose embedded timestamp
    falls within [start_dt, end_dt] (both endpoints inclusive).

    When a filter is active:
      - Files WITH a valid timestamp are included only if they are in-range.
      - Files WITHOUT a timestamp (e.g. L3_EVENT_X.dbg with no suffix) are
        EXCLUDED — they cannot be verified as in-range per the spec.
      - Files with a MALFORMED timestamp (invalid datetime values) are silently
        SKIPPED per spec: "If filename is malformed → ignore it."

    When NO filter is active (both bounds None), the full list is returned.

    Returns: (filtered_list, skipped_count, [])  — errors list is always empty;
    malformed files are simply counted as skipped.
    """
    if start_dt is None and end_dt is None:
        return list(file_list), 0, []
    ...
    for fname in file_list:
        try:
            ...
            # Malformed timestamp values → ignore per spec
            skipped += 1
            continue

        if ts is None:
            # No timestamp in filename: cannot verify it is within the requested
            # interval, so exclude it when any filter bound is active.
            skipped += 1
            continue
...
def _scp_download_and_analyze_worker(username, hostname, remote_dir, password, session_folder, start_dt=None, end_dt=None):
    """
    Background worker for SCP download + analysis so UI can poll /progress live.
    start_dt / end_dt (datetime | None): optional time-range filter applied to
    remote filenames before downloading.
    """
    global ue_data_map, rrc_counts, insights_global, valid_indices_global, total_ue_indices_global
    global last_analysis_snapshot, bt_df, crash, analysis_progress, latest_bt_text, rre_mapping_results, drx_messages, current_session_folder
    ...
    try:
        analysis_progress.update({
            ...
            'elapsed_time': 0,
            'next_url': '/results',
            # Reset filter fields so stale values from a prior session are not shown
            'filter_active': False,
            'filtered_total': 0,
            'total_before_filter': 0,
        })
        ...
        # CRITICAL: This filter determines EXACTLY which files will be downloaded.
        # NO files outside the filter will be copied to session folder.
        total_before_filter = len(remote_files)
        print(f"\n{'='*80}")
        print(f"📁 Total files found on server: {total_before_filter}")
        
        if start_dt is not None or end_dt is not None:
            print(f"⏰ Time filter active: {start_dt.strftime('%Y-%m-%d %H:%M:%S') if start_dt else 'START'} → {end_dt.strftime('%Y-%m-%d %H:%M:%S') if end_dt else 'END'}")
            analysis_progress['message'] = 'Filtering files by time range...'
            
            # Apply strict timestamp-based filter
            remote_files, _skipped, _ignored_errors = _filter_files_by_time_range(remote_files, start_dt, end_dt)
            
            print(f"✂️  Files filtered: {len(remote_files)} ACCEPTED, {_skipped} REJECTED")
            if len(remote_files) > 0:
                print(f"✅ Files to download: {', '.join([os.path.basename(f) for f in remote_files[:5]])}{'...' if len(remote_files) > 5 else ''}")
            
            if not remote_files:
                start_label = start_dt.strftime('%Y-%m-%d %H:%M:%S') if start_dt else '*'
                end_label   = end_dt.strftime('%Y-%m-%d %H:%M:%S')   if end_dt   else '*'
                print(f"❌ ERROR: No files found in time range [{start_label} — {end_label}]")
                print(f"{'='*80}\n")
                raise RuntimeError(
                    f"No files match the specified time range [{start_label} — {end_label}]."
                )
            
            # Update progress with filter metadata immediately so the UI reflects it
            analysis_progress['filter_active']      = True
            analysis_progress['filtered_total']     = len(remote_files)
            analysis_progress['total_before_filter'] = total_before_filter
        else:
            print(f"⚠️  No time filter applied - downloading ALL {total_before_filter} files")
            analysis_progress['filter_active']      = False
            analysis_progress['filtered_total']     = total_before_filter
            analysis_progress['total_before_filter'] = total_before_filter
        ...
        # ═══════════════════════════════════════════════════════════════════
        # CRITICAL SECTION: DOWNLOAD ONLY FILTERED FILES
        # Only files in remote_files (after filtering) will be downloaded.
        # NO other files will be copied to session_folder.
        # ═══════════════════════════════════════════════════════════════════
        
        sftp = ssh.open_sftp()
        print(f"🔽 Starting SCP download: {len(remote_files)} files")
        ...
        for idx, rf in enumerate(remote_files, 1):
            ...
            print(f"  [{idx}/{len(remote_files)}] Downloading: {local_name}")
            sftp.get(rf, local_path)
            downloaded.append(local_name)
        
        print(f"✅ Download complete: {len(downloaded)} files copied to {session_folder}")
        print(f"📦 Session folder contents: {', '.join(downloaded)}\n")
        ...
        analysis_progress.update({
            ...
            'elapsed_time': 0,
            'next_url': '/results',
            # Preserve filter metadata across the analysis phase
            'filter_active':       analysis_progress.get('filter_active', False),
            'filtered_total':      len(downloaded),
            'total_before_filter': analysis_progress.get('total_before_filter', len(downloaded)),
        })
        ...
        # ═══════════════════════════════════════════════════════════════════
        # VERIFICATION: Ensure session folder contains ONLY downloaded files
        # ═══════════════════════════════════════════════════════════════════
        actual_files = sorted([f for f in os.listdir(session_folder) 
                              if f.lower().startswith(('l3_event_x', 'rrm_event_x'))])
        expected_files = sorted(downloaded)
        
        if actual_files != expected_files:
            print(f"\n⚠️  WARNING: Session folder mismatch!")
            print(f"  Expected: {expected_files}")
            print(f"  Found:    {actual_files}")
            # Remove any unexpected files
            for unexpected in set(actual_files) - set(expected_files):
                unexpected_path = os.path.join(session_folder, unexpected)
                os.remove(unexpected_path)
                print(f"  🗑️  Removed unexpected file: {unexpected}")
        else:
            print(f"✅ Verification passed: {len(actual_files)} files in session folder match downloaded files\n")
        # ═══════════════════════════════════════════════════════════════════
        ...
        try:
            ...
            print("="*80)
            l3_rrm_correlator.initialize_correlation(session_folder, combined_map)
        ...
        last_analysis_snapshot = {
            ...
            'session_folder': session_folder,  # CRITICAL: Store session folder path for RRM correlation persistence
        }
        
        # CRITICAL: Store session folder globally for RRM correlation persistence
...
# =================================================================
# SCP PRE-CHECK: list + filter files, return count (used by UI warning dialog)
# =================================================================

@app.route("/scp_check_files", methods=["POST"])
def scp_check_files():
    """
    Connect to the remote server, list L3/RRM event files, apply the time-range
    filter, and return the resulting file count as JSON.
    Used by the frontend warning dialog before starting the actual download.
    """
    # ✅ DEBUG: Confirm this endpoint is being called
    print(f"\n{'='*80}")
    print(f"🔍 /scp_check_files CALLED")
    print(f"   Form data: {dict(request.form)}")
    print(f"{'='*80}\n")
    
    server_path   = request.form.get("server_path", "").strip()
    password      = request.form.get("server_password", "").strip()
    start_time_str = request.form.get("scp_start_time", "").strip()
    end_time_str   = request.form.get("scp_end_time", "").strip()
    
    print(f"   - server_path:     {server_path}")
    print(f"   - scp_start_time:  '{start_time_str}'")
    print(f"   - scp_end_time:    '{end_time_str}'")

    if not server_path or not password:
        return jsonify({"count": 0, "total": 0, "error": "Server path and password are required."})

    scp_re = re.match(r'^([^@]+)@([^:]+):(.+)$', server_path)
    if not scp_re:
        return jsonify({"count": 0, "total": 0,
                        "error": "Invalid server path format. Use: user@host:/path/to/logs"})
    ...
    try:
        ...
        return jsonify({"count": 0, "total": 0,
                        "error": f"Invalid time format: {exc}. Use YYYY-MM-DD HH:MM:SS"})
    ...
    try:
        ...
        stdin, stdout, stderr = ssh.exec_command(
            f'ls {remote_dir}/L3_EVENT_X.dbg* {remote_dir}/RRM_EVENT_X.dbg* 2>/dev/null'
        )
        remote_files = [ln.strip() for ln in stdout.read().decode().splitlines() if ln.strip()]

        if not remote_files:
            err = stderr.read().decode().strip()
            return jsonify({"count": 0, "total": 0,
                            "error": f"No matching files found at {remote_dir}. {err}".strip()})

        filtered, _skipped, _ignored_errors = _filter_files_by_time_range(remote_files, start_dt, end_dt)

        return jsonify({"count": len(filtered), "total": len(remote_files), "error": None})

    except Exception as exc:
        return jsonify({"count": 0, "total": 0, "error": str(exc)})
    finally:
        try:
            if ssh:
                ssh.close()
...
def internal_error(exc):
    """Log the full traceback to the console and return a readable error page."""
    import traceback as _tb
    _tb.print_exc()
    msg = f"{type(exc).__name__}: {exc}"
    return render_template("error.html", error=f"Internal Server Error — {msg}"), 500
...
def analyze_logs(folder):
    ...
    """
    global ue_data_map, rrc_counts, insights_global, valid_indices_global, total_ue_indices_global
    global last_analysis_snapshot, bt_df, crash, analysis_progress, current_session_folder
    ...
    analysis_progress.update({
        ...
        'elapsed_time': 0,
        'next_url': '/results',
        # No filter in this path (direct folder analysis without time range)
        'filter_active': False,
        'filtered_total': 0,
        'total_before_filter': 0,
    })
    ...
    try:
        ...
        last_analysis_snapshot = {
            ...
            'rre_edge_case_mappings_global': rre_edge_case_mappings_global.copy() if rre_edge_case_mappings_global else [],  # Add RRE edge case mappings to snapshot
            'drx_messages': drx_messages.copy(),  # Add DRX messages to snapshot
            'session_folder': resolved_folder,  # CRITICAL: Store session folder path for RRM correlation persistence
        }
        
        # CRITICAL: Store session folder globally for RRM correlation persistence
        current_session_folder = resolved_folder
    ...
    if request.method == "POST":
        ...
        if action == "analyze":
            ...
            for f in files:
                ...
                if ((fn_lower.startswith("l3_event_x") or fn_lower.startswith("rrm_event_x")) 
                    and (".dbg" in fn_lower or ".bkp" in fn_lower)):
            ...
            _local_filter_active      = False
            _local_total_before       = len(saved_files)
            
            print(f"\n{'='*80}")
            print(f"📁 Local upload: {_local_total_before} files saved initially")
            
            if start_dt is not None or end_dt is not None:
                print(f"⏰ Time filter active: {start_dt.strftime('%Y-%m-%d %H:%M:%S') if start_dt else 'START'} → {end_dt.strftime('%Y-%m-%d %H:%M:%S') if end_dt else 'END'}")
                
                filtered_names, _skipped, _ignored_errors = _filter_files_by_time_range(
                    saved_files, start_dt, end_dt
                )
                
                print(f"✂️  Files filtered: {len(filtered_names)} ACCEPTED, {_skipped} REJECTED")
                
                # Remove files that did not pass the filter from disk
                removed_size = 0.0
                removed_count = 0
                for fname in saved_files:
                    if fname not in filtered_names:
                        fp = os.path.join(session_folder, fname)
                        try:
                            removed_size += os.path.getsize(fp) / (1024 * 1024)
                            os.remove(fp)
                            removed_count += 1
                            print(f"  🗑️  Deleted (out of range): {fname}")
                        except Exception as e:
                            print(f"  ⚠️  Failed to delete {fname}: {e}")
                
                saved_files = filtered_names
                total_size_mb = max(0.0, total_size_mb - removed_size)
                _local_filter_active = True
                
                print(f"✅ {removed_count} files removed, {len(saved_files)} files remain in session folder")
                if len(saved_files) > 0:
                    print(f"📦 Remaining files: {', '.join(saved_files[:5])}{'...' if len(saved_files) > 5 else ''}")
                print(f"{'='*80}\n")
                if not saved_files:
                    shutil.rmtree(session_folder, ignore_errors=True)
                    return _upload_page_context(
                        error="No uploaded files matched the specified time range."
                    )
            # ─────────────────────────────────────────────────────────────
            ...
            analysis_progress.update({
                ...
                'elapsed_time': 0,
                'next_url': '/results',
                # Filter metadata for Progress page display
                'filter_active':       _local_filter_active,
                'filtered_total':      len(saved_files),
                'total_before_filter': _local_total_before,
            })
        ...
        if action == "scp_analyze":
            ...
            # NOTE: SCP uses scp_start_time/scp_end_time to avoid conflict with local upload fields
            start_time_str = request.form.get("scp_start_time", "").strip()
            end_time_str   = request.form.get("scp_end_time", "").strip()
            
            # ✅ DEBUG: Print received time filter values
            print(f"\n{'='*80}")
            print(f"🕒 TIME FILTER DEBUG - SCP Upload")
            print(f"   Raw form data received:")
            print(f"   - scp_start_time field: '{start_time_str}' (length: {len(start_time_str)})")
            print(f"   - scp_end_time field:   '{end_time_str}' (length: {len(end_time_str)})")
            print(f"{'='*80}\n")
            ...
            try:
                ...
                if start_dt or end_dt:
                    print(f"✅ Time filter ACTIVE - will pass to worker thread")
                else:
                    print(f"⚠️  No time filter - both start_dt and end_dt are None")
                print()
            ...
            # ✅ DEBUG: Confirm what's being passed to worker thread
            print(f"🚀 Starting worker thread with parameters:")
            print(f"   - start_dt: {start_dt}")
            print(f"   - end_dt:   {end_dt}")
            print(f"   - Filter will be {'ACTIVE' if (start_dt or end_dt) else 'INACTIVE'}")
            print()

            worker = threading.Thread(
                target=_scp_download_and_analyze_worker,
                args=(username, hostname, remote_dir, password, session_folder, start_dt, end_dt),
                daemon=True
            )
            worker.start()
...
def ue_journey(ue_index):
    ...
    # CRITICAL: Ensure RRM correlation data is loaded before trying to access it
    _ensure_rrm_correlation_loaded()
...
def _ensure_rrm_correlation_loaded():
    """
    Ensure RRM correlation data is loaded for the current session.
    Reloads from session folder if correlator is empty but session folder is available.
    """
    global current_session_folder, last_analysis_snapshot
    
    # Check if correlator already has data
    corr_stats = l3_rrm_correlator.get_correlation_stats()
    if corr_stats['rrm_log_lines'] > 0:
        print(f"✅ RRM correlation data already loaded: {corr_stats['rrm_log_lines']} lines, {corr_stats['ues_with_rrm']} UEs")
        return True
    
    # Try to get session folder from current_session_folder or snapshot
    session_folder = current_session_folder
    if not session_folder and last_analysis_snapshot:
        session_folder = last_analysis_snapshot.get('session_folder')
    
    if not session_folder or not os.path.isdir(session_folder):
        print(f"⚠️ Cannot reload RRM data: session folder not available")
        return False
    
    # Reload RRM correlation data
    print(f"\n🔄 Reloading RRM correlation data from: {session_folder}")
    try:
        ue_data = _get_ue_data_map()
        l3_rrm_correlator.initialize_correlation(session_folder, ue_data)
        corr_stats = l3_rrm_correlator.get_correlation_stats()
        print(f"✅ RRM correlation reloaded: {corr_stats['rrm_log_lines']} lines, {corr_stats['ues_with_rrm']} UEs")
        return corr_stats['rrm_log_lines'] > 0
    except Exception as e:
        print(f"⚠️ Failed to reload RRM correlation: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def open_browser() -> None:
    """Open the default browser after Flask has had time to bind the socket."""
    time.sleep(1.5)
...
if __name__ == "__main__":
    ...
    app.run(
        ...
        port=5000,
        debug=True,
        use_reloader=True,   # Required for PyInstaller --onefile
        threaded=True,
    )